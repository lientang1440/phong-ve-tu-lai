-- Production-grade SQL for Supabase
-- This script creates a clean relational schema for the current website data.
...
create index if not exists idx_routes_from_to on public.routes(from_port, to_port);
create index if not exists idx_departures_route on public.departures(route_id);
create index if not exists idx_departures_time on public.departures(departure_time);
create index if not exists idx_blog_category on public.blog_posts(category);
create index if not exists idx_faq_category on public.faqs(category);
...
-- Optional: enable read access for anonymous users (adjust if needed)
...
-- Example policies if you want to expose read-only data publicly
-- create policy "Public can read routes" on public.routes for select using (true);
-- create policy "Public can read departures" on public.departures for select using (true);
